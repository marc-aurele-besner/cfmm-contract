...
describe("SplitFeeCFMM - Fees", function () {
  let signers: Signers;
  ...
  beforeEach(async function () {
    ...
    await fixture.tokenA.connect(signers.alice).transfer(await fixture.pair.getAddress(), amountA);
    await fixture.tokenB.connect(signers.alice).transfer(await fixture.pair.getAddress(), amountB);
  ...
  it("Should allow user to claim fees", async function () {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      // Use 50% of CURRENT reserves for maximum fee generation
  ...
  it("Should handle fee claiming after adding liquidity", async function () {
    ...
    if (accumulatedFee > 0n) {
      ...
      await fixture.tokenB.connect(signers.bob).approve(await fixture.pair.getAddress(), amountBIn2 * 2n);
      await fixture.pair.connect(signers.bob).swap(amountAOut2, 0n, signers.bob.address);
